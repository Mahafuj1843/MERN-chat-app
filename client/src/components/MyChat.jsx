...
const MyChat = ({ myChats, dispatch }) => {
    ...
    return (
            ...
            <div className="flex flex-col border-r border-neutral-700 w-full h-[89.5vh]">
                ...
                <div className="flex space-x-2 items-center w-full h-[60px] p-2 border-b">
                    <div className="flex-grow flex items-center space-x-2 bg-gray-200 py-1 px-4 rounded-md">
                        ...
                        <input onKeyUp={TextSearch} type="search" className="focus:outline-none bg-gray-200 w-full text-gray-500 text-xs"
                            placeholder="Search chat" />
                    </div>
                </div>
                <div className="flex flex-col overflow-y-scroll cursor-pointer w-full h-full">
                    {
                        myChats?.map((items, i) => {
                            return (
                                <button key={i} onClick={accessChatMsg.bind(this, items)} className={`flex items-center w-full px-4 py-2 border-b transition-colors duration-200 gap-x-3 ${selectUser?._id === items._id ? "bg-gray-200" : "bg-white hover:bg-gray-200"} dark:hover:bg-gray-800 focus:outline-none`}>
                                    ...
                                    <div className='flex justify-between w-full'>
                                        <div className="text-left rtl:text-right space-y-1">
                                            <h1 id='ChatName' className="text-sm font-medium text-gray-700 capitalize dark:text-white ">
                                                {items.isGroupChat ?
                                        ...
                                        <div className="flex flex-col flex-none text-xs items-end space-y-2 w-20 pr-2">
                                            <p className={`${notifications.find((n) => n.chat._id === items._id) ? 'text-[#00a884] font-medium' : 'text-gray-500 dark:text-gray-400'} text-xs`}>{moment(items.latestMessage?.createdAt).format("DD/MM/YYYY")}</p>
        ...
        // <Fragment>
        //     <div className={`${selectUser && 'hidden lg:block'} flex flex-col w-full lg:w-1/3 border-r overflow-hidden`}>
        ...
        //             <div className="flex flex-col space-y-4 flex-grow mt-2">
        // <div className="flex space-x-2 items-center px-4 pb-2 border-b">
        //     <div className="flex-grow flex items-center space-x-2 bg-gray-200 py-1 px-4 rounded-md">
        //         <svg viewBox="0 0 24 24" width="24" height="24" className="cursor-pointer">
        //             <path fill="text-gray-700"
        //                 d="M15.009 13.805h-.636l-.22-.219a5.184 5.184 0 0 0 1.256-3.386 5.207 5.207 0 1 0-5.207 5.208 5.183 5.183 0 0 0 3.385-1.255l.221.22v.635l4.004 3.999 1.194-1.195-3.997-4.007zm-4.808 0a3.605 3.605 0 1 1 0-7.21 3.605 3.605 0 0 1 0 7.21z">
        ...
        //                 <div id='chat' className="overflow-x-hidden overflow-y-auto h-4/5 lg:h-[88%] xl:max-h-[489px]">
        // {
        //     myChats?.map((items, i) => {
        //         return (
        //             <button key={i} onClick={accessChatMsg.bind(this, items)} className={`flex items-center w-full px-4 py-2 border-b transition-colors duration-200 gap-x-3 ${selectUser?._id===items._id ? "bg-gray-200":"bg-white hover:bg-gray-200"} dark:hover:bg-gray-800 focus:outline-none`}>
