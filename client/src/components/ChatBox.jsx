...
import { setSingleMessage } from '../redux/state/chatSlice'
import { myChatRequest } from '../apiRequest/chatRequset'
import { setNotification } from '../redux/state/settingSlice'
...
const ChatBox = ({ dispatch }) => {
    ...
    const onTyping = (e) => {
        ...
        if (!typing) {
            setTyping(true);
            socket.emit("typing", selectUser._id);
        }
            ...
            if(renderRun.current===false){
                socket.on("msg recieved", (newMsgRecieved) => {
                    if (!selectedChatCompare || selectedChatCompare._id !== newMsgRecieved.chat._id) {
                        console.log(notifications.includes(newMsgRecieved))
    ...
    return (
        <Fragment>
            {
                selectUser &&
                <div class="flex flex-col bg-white overflow-hidden flex-grow"
                    style={{ backgroundImage: BG }}>
                    <div class="flex justify-between bg-white px-4 py-2 border-b-2">
                        <div class="flex flex-none space-x-2 items-center">
                            ...
                            <div class="flex flex-col">
                                ...
                                <span class="text-sm text-[#777e83]">last seen 19/05/2022 at 03:56 pm</span>
                            </div>
                        </div>
                    ...
                    </div>
                    {
                        loading ?
                            ...
                            <div class="flex flex-col w-full max-h-auto relative bg-black-rgba scroller overflow-x-hidden overflow-y-auto flex-grow">
                                <Message messages={allMessage} isTyping={isTyping}/>
                                
                            </div>
                    }
                    <div class="flex space-x-2 items-center bg-white px-6 py-2 border-t">
                        <div class="flex space-x-4 cursor-pointer">
                            <svg viewBox="0 0 24 24" width="28" height="28" class="ekdr8vow dhq51u3o">
                                <path fill="text-gary-700"
                            ...
                            <svg viewBox="0 0 24 24" width="28" height="28" class="cursor-pointer">
                                <path fill="text-gary-700"
