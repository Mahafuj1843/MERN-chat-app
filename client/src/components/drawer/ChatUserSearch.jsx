    ...
    <Fragment >
      <aside className="fixed h-full w-full z-50 left-0 top-0 transition duration-300 ease-in-out" style={{ display: show ? 'block' : 'none' }}>
        ...
        <div className="fixed h-full w-3/4 md:w-2/6 xl:w-1/5 left-0 top-0 bg-white shadow-lg pt-8 p-x2 transition duration-700 ease-in-out" style={{ left: show ? '0%' : '100%' }}>
          <h2 className="px-5 text-lg font-medium text-gray-800 dark:text-white">Search Users</h2 >
          <div className="flex gap-2 my-4 px-5">
            <input ref={(i) => search = i} type="search" id="search" className="bg-gray-50 border border-[#0C7075] text-gray-900 text-sm rounded-lg focus:ring-[#0C7075] focus:border-[#15848a] block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-[#0C7075] dark:focus:border-[#1d99a0]" placeholder="e.g. Jhon" required />
          ...
          {loading ?
            <UserSearchLoadings />
            :
            <div className="mt-6 space-y-1 h-4/5 overflow-y-auto">
              {
                searchUsers?.map((user, i) => {
                  return (
                    <button key={i} onClick={accessChat.bind(this, user._id)} className="flex items-center w-full px-5 py-2 transition-colors duration-200 dark:hover:bg-gray-800 gap-x-2 hover:bg-gray-100 focus:outline-none">
